...
var followMouse = function followMouse(item, mouse, k) {
...
var rotateLine = function rotateLine(item, radio, r) {
  var theta = angle(r);
  var x1 = item.cx + radio * Math.cos(theta);
  var y1 = item.cy + radio * Math.sin(theta);
  var x2 = item.cx - radio * Math.cos(theta);
  var y2 = item.cy - radio * Math.sin(theta);
  return {
    x1: x1,
    y1: y1,
    x2: x2,
